...
modF_disc <- read_csv("output/discrete_continuous_model_F_sim.csv")
...
# no conidia
# otherwise rerun discrete F without conidia (need disease-free)
...
modDis_disc <- disc_AFP_mod(A0 = filter(modInv_disc_fin, species == "Annual")$N, 
                            ...
                            disc_parms = disc_params, cont_parms = cont_params)
write_csv(modDis_disc, "output/discrete_continuous_model_invasion_disease.csv")
modDis_disc <- read.csv("output/discrete_continuous_model_invasion_disease.csv")
...
# annual establishes without disease
...
modA_healthy_disc <- read_csv("output/discrete_continuous_model_A_healthy_sim.csv")
...
mod_invP <- modA_healthy_disc %>%
  full_join(modInvP_disc %>%
              mutate(time = time + max(modA_healthy_disc$time)))
...
# starting values
modInv_disc_fin <- read.csv("output/discrete_continuous_model_invasion_no_disease.csv") %>%
  filter(time == max(time)/2)
...
# Note: re-importing affects rounding, cannot filter using alpha_AA_PA_vals
# as done below
...
# Note: re-importing affects rounding, cannot filter using alpha_AA_PP_vals
# as done below
